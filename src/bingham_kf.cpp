...
BinghamKFResult *bingham_kf(Vector4ld *Xk, Matrix4ld *Mk, Matrix4ld *Zk, 
	...
	Matrix4ld DStar = -0.5 * D1 + (*Mk) * (*Zk) * (*Mk).transpose();
...
struct BinghamKFResult *bingham_normal_kf(Vector4ld *Xk, Matrix4ld *Mk, Matrix4ld *Zk, 
	...
	VectorXld s = es.eigenvalues().real();  // A vector whose entries are eigen values of RTmp
	MatrixXld U = es.eigenvectors().real();	// A matrix whose column vectors are eigen 
										    // vectors of RTmp
	...
	VectorXld ZTmp = es.eigenvalues().real();  // A vector whose entries are eigen values of RTmp
	MatrixXld MTmp = es.eigenvectors().real();	// A matrix whose column vectors are 
												// eigen vectors of RTmp
    ...
    // Estimated pose parameters  (x,y,z,theta_x, theta_y, theta_z)
    VectorXld Xreg(6);
    Xreg.segment(0,3) = centroidDifference;
    Xreg.segment(3,3) = eulerRotation;
